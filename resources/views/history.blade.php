...
<body>
    ...
    @php
        $roomchat = \App\Models\RoomChat::where(function ($query) use ($username) {
            $query->where('user1', $username)->orWhere('user2', $username);
        })
            ->where('leave', 1)
            ->get();
    @endphp
            ...
            <div class="row">
                ...
                <div class="col-lg-8 col-md-12 mb-12">
                    <div class="messages" id="messageContainer" style="height: 510px; overflow-y: auto;">
                        @if ($roomchat->count() > 0)
                            @foreach ($roomchat->reverse() as $room)
                                <div class="media-body">
                                    @php
                                        $messageCount = \App\Models\Message::where('inroom', $room->id)->count();
                                    @endphp
                                    <div style=""><a href="/history/{{ $room->id }}">Mã phòng:
                                            {{ $room->id }}</a></div>
                                    <small class="text-muted">Số tin nhắn: {{ $messageCount }} </small> <br>
                                    <small class="text-muted">Kết thúc lúc: {{ $room->timestamp }} </small>
                            ...
                            @endforeach
                        @else
                            Bạn chưa có cuộc trò chuyện nào
                        @endif
                    </div>
                </div>
            </div>
